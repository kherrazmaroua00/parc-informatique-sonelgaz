...
export default function EquipementFilterBar({
  ...
  return (
    <div className="bg-white rounded-xl border border-black-400 p-4 mb-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-3">
          {activeFilterCount > 0 && (
            <span className="text-xs bg-gray-100 text-black px-2.5 py-1 rounded-full">
              {activeFilterCount} filtre{activeFilterCount > 1 ? 's' : ''} applique{activeFilterCount > 1 ? 's' : ''}
            </span>
          )}
          <button
            onClick={onReset}
            className="flex items-center gap-1 text-xs text-black hover:text-gray-800"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div>
          ...
          <select
            ...
            className="w-full text-sm text-gray-900 border border-gray-200 rounded-lg px-3 py-2 outline-none bg-white"
          >
